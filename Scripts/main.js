...
function displayEq(equation){
  adjustFontSize()
  document.querySelector('.display').innerHTML = equation.replace(/\*/g,"×").replace(/\//g,"÷")
}
function roundUp(num) {
    if (num >= 0) {
      document.querySelector('.display-approx').innerHTML = Math.ceil(num * 1000) / 1000;
    } else {
      document.querySelector('.display-approx').innerHTML = Math.floor(num * 1000) / 1000 ;
    }
}
